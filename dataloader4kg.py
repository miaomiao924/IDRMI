...
class Ml_100K():
    __BASE = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'Data/moocCube')
...
def readRecData(path,test_ratio=0.2):
    ...
    train_set = list(data_generator.train_triples)
    eval_set = list(data_generator.eval_triples)
    test_set = list(data_generator.test_triples)
    ...
    # print('test',test_set)
    # print('list',list(item_set))
    #返回用户集合列表，物品集合列表，与用户，物品，评分三元组列表
    return list(user_set),list(item_set),train_set,eval_set,test_set
